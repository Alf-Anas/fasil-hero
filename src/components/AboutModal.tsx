...
import { X, Award, ShieldCheck, Database, HelpCircle, Layers, CheckCircle, ExternalLink, Sparkles } from 'lucide-react';
...
export const AboutModal: React.FC<AboutModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl p-6 max-w-2xl w-full shadow-2xl border border-slate-200 relative my-8 space-y-6 max-h-[85vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-100 pb-4 sticky top-0 bg-white z-10 pt-2">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white font-black text-xl flex items-center justify-center shadow-md">
              F
            </div>
            <div>
              <h2 className="text-xl font-black text-slate-900">Tentang FasilHero</h2>
        ...
        <div className="space-y-3 pt-3 border-t border-slate-100">
          ...
          <p className="text-xs text-slate-600 leading-relaxed">
            ...
            <code className="bg-slate-100 text-blue-700 px-2 py-0.5 rounded font-mono font-bold">
              SUM(Jumlah Lencana Keahlian) + SUM(Jumlah Arcade Game)
            </code>{' '}
            ...
            {FACILITATOR_MILESTONES.map((m) => (
              <div key={m.id} className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                <div className="flex items-center justify-between text-xs font-bold text-slate-800">
                  <span>{m.title}</span>
                  <span className="text-blue-600">{m.targetCombined.toLocaleString()} Poin</span>
                </div>
        ...
        <div className="space-y-3 pt-3 border-t border-slate-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-slate-600 font-mono bg-slate-50 p-3 rounded-xl border border-slate-200">
        ...
        <div className="flex justify-end pt-3 border-t border-slate-100">
          <button
            onClick={onClose}
            className="px-5 py-2 text-xs font-extrabold text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-md shadow-blue-500/20"
          >
            Mengerti & Tutup
          </button>
        </div>
