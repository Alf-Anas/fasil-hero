...
export const UploadModal: React.FC<UploadModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="bg-slate-900 rounded-3xl p-5 sm:p-6 max-w-xl w-full shadow-2xl border border-slate-800 relative my-8 space-y-5">
        {/* Header */}
        ...
        <div className="bg-blue-500/10 p-3.5 rounded-2xl border border-blue-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-600 text-white rounded-xl shrink-0">
              <FileText className="w-5 h-5" />
            </div>
            <div>
              <p className="text-xs font-bold text-blue-300">Belum Punya Format File Laporan?</p>
              <p className="text-[11px] text-blue-200/80">Download template Excel resmi dengan header kolom standar.</p>
            </div>
          </div>
          <button
            type="button"
            onClick={downloadSampleExcelTemplate}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 text-blue-300 font-bold text-xs rounded-xl border border-blue-500/30 hover:bg-slate-700 transition-colors shrink-0 shadow-xs cursor-pointer"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full p-3 text-xs bg-slate-950 border border-slate-800 rounded-xl focus:outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-100"
              required
            />
          </div>
          ...
          <div>
            ...
            >
              ...
              <label htmlFor="file-upload-input" className="cursor-pointer space-y-2 block">
                <div className="w-12 h-12 mx-auto rounded-full bg-slate-800 text-blue-400 flex items-center justify-center border border-slate-700">
                  <Upload className="w-6 h-6" />
                </div>
                {selectedFile ? (
                  <div>
                    <p className="text-xs font-extrabold text-emerald-400">{selectedFile.name}</p>
                  ...
                  <div>
                    <p className="text-xs font-bold text-slate-200">
                      Tarik & Lepas File di Sini, atau <span className="text-blue-400 underline">Pilih File</span>
                    </p>
          ...
          <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 text-xs font-bold text-slate-300 rounded-xl border border-slate-800 hover:bg-slate-800 transition-colors"
            >
              Batal
            </button>
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-2.5 text-xs font-extrabold text-white bg-blue-600 rounded-xl hover:bg-blue-500 shadow-md shadow-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer"
            >
              {isProcessing ? (
                <>
